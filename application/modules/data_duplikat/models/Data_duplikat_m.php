...
class Data_duplikat_m extends CI_Model
{
    ...
    {
        $this->db->select('mahasiswa_beasiswa.*, COUNT(nim_mahasiswa) as total_duplikat');
        $this->db->from('mahasiswa_beasiswa');
        $this->db->group_by('nim_mahasiswa');
        $this->db->having('COUNT(nim_mahasiswa) >', '2');
        $i = 0;
