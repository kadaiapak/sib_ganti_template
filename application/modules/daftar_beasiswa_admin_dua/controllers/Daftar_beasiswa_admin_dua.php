...
class Daftar_beasiswa_admin_dua extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model('Daftar_beasiswa_admin_dua_m', 'daftar_beasiswa_admin_dua');
    ...
    {
        $list = $this->daftar_beasiswa_admin_dua->get_datatables($id, $display);
        $data = array();
        $no = @$_POST['start'];
        foreach ($list as $item) {
            ...
            $row[] = '<a href="'.site_url('daftar-beasiswa-admin-dua/detail-mahasiswa/'.$id.'/'.$item->nim_mahasiswa).'" class="btn btn-primary btn-xs"><i class="fas fa-search-plus"></i> Detail</a>';
            $data[] = $row;
        }
        $output = array(
                    "draw" => @$_POST['draw'],
                    "recordsTotal" => $this->daftar_beasiswa_admin_dua->count_all($id, $display),
                    "recordsFiltered" => $this->daftar_beasiswa_admin_dua->count_filtered($id, $display),
                    "data" => $data,
                );
    ...
    public function index()
    {
        ...
        $data['master_beasiswa'] = $this->daftar_beasiswa_admin_dua->getMasterBeasiswaDaftar()->result_array();
        $data['isi'] = 'daftar_beasiswa_admin_dua_v';
    ...
    {        
        ...
        if($this->form_validation->run() == false){
            $data['isi'] = 'daftar_beasiswa_admin_detail_beasiswa_dua_v';
            $this->load->view('template/wrapper_frontend_v', $data);
        }else {
    ...
    public function input_data($id= null)
    {
        ...
        foreach ($persyaratanUtama as $pr) {
            ...
            // koding ini digunakan agar semua file harus di upload oleh mahasiswa
            // jika ada file yang tidak di uppload maka proses upload jadi gagal
            if($wajib == '1'){
        ...
        if($this->form_validation->run() == false){
            ...
            {   
                ...
                $this->session->set_flashdata("error_upload", 
                    "Gagal Upload, ada kesalahan dalam ukuran, dan perhatikan jenis file yang dibutuhkan, apalah itu JPG atau PDF dll");
                redirect('daftar-beasiswa-admin-dua/input-data/'.$id);
            }
            ...
            if($this->db->affected_rows() > 0){
                ...
                redirect('daftar-beasiswa-admin-dua/detail/'.$id);
            }
        }
    }
    ...
    {
        $data['user_created'] = $this->daftar_beasiswa_admin_dua->getAdminPendaftar($id, $nim)->row()->admin_pendaftar;
        $data['cek_akses_user'] = $this->cek_akses_user;
        $data['mahasiswa'] = $this->daftar_beasiswa_admin_dua->getMahasiswaPendaftar($id, $nim)->row();
        ...
        $data['isi'] = 'daftar_beasiswa_admin_detail_mahasiswa_dua_v';
    ...
    {
            $post = $this->input->post(null, TRUE);
            $pernah = $this->daftar_beasiswa_admin_dua->cekSedangDapatBeasiswaModel($post['nim']);
            if($pernah > 0) {
    ...
    {
        $post = $this->input->post(null, TRUE);
        $hasil = $this->daftar_beasiswa_admin_dua->cekPernahDaftarBeasiswaModel($post['nim'], $post['id_beasiswa']);
        if($hasil > 0) {
